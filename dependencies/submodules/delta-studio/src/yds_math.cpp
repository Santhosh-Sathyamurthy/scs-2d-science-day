...
// FIXED: Cross-platform quaternion component extraction using _mm_store_ps
...
ysMatrix ysMath::LoadMatrix(const ysQuaternion &quat) {
    ...
    // Assembly
    ysGeneric asm1 = _mm_shuffle_ps(calc2, calc3, _MM_SHUFFLE(0, 2, 3, 0));
    asm1 = _mm_shuffle_ps(asm1, asm1, _MM_SHUFFLE(1, 3, 2, 0));

    ysGeneric asm2 = _mm_shuffle_ps(calc2, calc3, _MM_SHUFFLE(3, 1, 3, 1));
    asm2 = _mm_shuffle_ps(asm2, asm2, _MM_SHUFFLE(1, 3, 0, 2));

    ysGeneric asm3 = _mm_shuffle_ps(calc1, calc2, _MM_SHUFFLE(3, 2, 3, 0));
...
ysMatrix ysMath::LoadMatrix(const ysQuaternion &quat, const ysVector &origin) {
    ysGeneric q = ysMath::Normalize(quat);
    ...
    ysGeneric i1 = _mm_mul_ps(xxxy, zyyz);
    ysGeneric i2 = _mm_mul_ps(yzzx, wwww);
...
ysVector ysMath::Det3x3(const ysMatrix &m) {
    ...
    ysVector interior = _mm_sub_ps(_mm_mul_ps(c0, c1), _mm_mul_ps(c2, c3));
...
ysMatrix ysMath::Inverse3x3(const ysMatrix &m) {
    ...
    return LoadMatrix(row1, row2, row3, Constants::Zero);
}
...
ysVector ysMath::Clamp(const ysVector &a, const ysVector &r_min, const ysVector &r_max) {
    return ComponentMax(ComponentMin(a, r_max), r_min);
}
