...
// The key-map only makes sense on Windows (Virtual Key codes are Windows-only).
// On Linux/macOS the SDL2 backend translates SDL_Scancode directly; the
// Windows key-map array is still allocated but left as "Undefined" so that
// any accidental calls do not crash.
...
const ysKey::Code *ysKeyMaps::GetWindowsKeyMap() {
    if (m_windowsKeyMap == nullptr) {
        m_windowsKeyMap = new ysKey::Code[256];
        for (int i = 0; i < 256; ++i)
            m_windowsKeyMap[i] = ysKey::Code::Undefined;
        ...
        // 0–9
        for (int i = 0; i < 10; ++i)
            m_windowsKeyMap[0x30 + i] = (ysKey::Code)((int)ysKey::Code::N0 + i);

        // A–Z
        for (int i = 0; i < 26; ++i)
            m_windowsKeyMap[0x41 + i] = (ysKey::Code)((int)ysKey::Code::A + i);
        ...
        // Numpad
        for (int i = 0; i < 10; ++i)
            m_windowsKeyMap[VK_NUMPAD0 + i] = (ysKey::Code)((int)ysKey::Code::Numpad0 + i);
        ...
        // F-keys
        for (int i = 0; i < 24; ++i)
            m_windowsKeyMap[VK_F1 + i] = (ysKey::Code)((int)ysKey::Code::F1 + i);
        ...
        // On non-Windows platforms the array stays as Undefined —
        // the SDL input backend builds its own mapping independently.
    }
