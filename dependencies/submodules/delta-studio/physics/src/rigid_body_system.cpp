// Removed Windows.h and <process.h> — both are Windows-only and unused here.
// Portable replacements: <ctime> and <cassert> are standard C++.
...
void dphysics::RigidBodySystem::OrderPrimitives(
    CollisionObject **prim1, CollisionObject **prim2,
...
void dphysics::RigidBodySystem::ProcessGridCell(int x, int y) {
    ...
    if (gridCell->m_valid && !gridCell->m_processed) {
        ...
        for (int i = 0; i < cellObjects; i++) {
            RigidBody *body1 = gridCell->m_objects[i];
            for (int j = i + 1; j < cellObjects; j++) {
                RigidBody *body2 = gridCell->m_objects[j];
...
void dphysics::RigidBodySystem::GenerateCollisions(int start, int count) {
    ...
    for (auto cell : m_gridPartitionSystem.m_gridCells) {
        ...
        if (gridCell->m_valid &&
            (gridCell->m_forceProcess || gridCell->GetRequestCount() >= REQUEST_THRESHOLD))
        {
            int cellObjects = gridCell->m_objects.GetNumObjects();
            for (int i = 0; i < cellObjects; i++) {
                RigidBody *body1 = gridCell->m_objects[i];
    ...
    for (int i = 0; i < rigidBodyCount; ++i) delete[] visited[i];
    delete[] visited;
}
    ...
    for (int i = 0; i < nPrim1; i++) {
        for (int j = 0; j < nPrim2; j++) {
            ...
            if (mode1 == mode2) {
                ...
                if (!coarseCollision && coarsePresent &&
                    mode1 == CollisionObject::Mode::Fine) continue;
                ...
                for (int k = 0; k < nCollisions; ++k) {
                    ...
                    *newCollisionEntry = newCollisions[k];
...
void dphysics::RigidBodySystem::ResolveCollision(
    Collision *collision, ysVector *velocityChange, ysVector *rotationDirection,
    float rotationAmount[2], float penetration)
{
    ...
    if (collision->m_bodies[1] != nullptr) {
        ...
        float maxAng = angularLimit * ysMath::GetScalar(ysMath::Magnitude(collision->m_relativePosition[0]));
        if (std::abs(angularMove[1]) > maxAng) {
            float pp = angularMove[1] + linearMove[1];
            angularMove[1] = angularMove[1] > 0 ? maxAng : -maxAng;
    ...
    float maxAng = angularLimit * ysMath::GetScalar(ysMath::Magnitude(collision->m_relativePosition[0]));
    if (std::abs(angularMove[0]) > maxAng) {
    ...
    for (b = 0; b < 2; b++) {
        if (collision->m_bodies[b] != nullptr) {
            ...
            if (angularMove[b] != 0.0f) {
                ...
                ysMatrix inverseInertiaTensor = body->GetInverseInertiaTensorWorld();
            ...
            velocityChange[b] = ysMath::Mul(collision->m_normal,
                                            ysMath::LoadScalar(linearMove[b] / rotationAmount[b]));
...
void dphysics::RigidBodySystem::AdjustVelocities(float timestep) {
    ...
    for (int velocityIterationsUsed = 0;
         velocityIterationsUsed < ResolutionIterationLimit;
        ...
        for (int i = 0; i < numContacts; ++i) {
            ...
            if (c->m_bodies[0] != nullptr) {
                if (c->m_bodies[0] == biggestCollision->m_bodies[0]) {
                    cp = ysMath::Add(ysMath::Cross(rotationChange[0], c->m_relativePosition[0]), velocityChange[0]);
                    ...
                    cp = ysMath::Add(ysMath::Cross(rotationChange[1], c->m_relativePosition[0]), velocityChange[1]);
                    c->m_contactVelocity = ysMath::Add(c->m_contactVelocity,
                        ysMath::MatMult(ysMath::OrthogonalInverse(c->m_contactSpace), cp));
                    c->CalculateDesiredDeltaVelocity(timestep);
                }
            ...
            if (c->m_bodies[1] != nullptr) {
                if (c->m_bodies[1] == biggestCollision->m_bodies[0]) {
                    cp = ysMath::Add(ysMath::Cross(rotationChange[0], c->m_relativePosition[1]), velocityChange[0]);
                    c->m_contactVelocity = ysMath::Sub(c->m_contactVelocity,
                        ysMath::MatMult(ysMath::OrthogonalInverse(c->m_contactSpace), cp));
                    c->CalculateDesiredDeltaVelocity(timestep);
                } else if (c->m_bodies[1] == biggestCollision->m_bodies[1]) {
                    cp = ysMath::Add(ysMath::Cross(rotationChange[1], c->m_relativePosition[1]), velocityChange[1]);
                    c->m_contactVelocity = ysMath::Sub(c->m_contactVelocity,
                        ysMath::MatMult(ysMath::OrthogonalInverse(c->m_contactSpace), cp));
                    c->CalculateDesiredDeltaVelocity(timestep);
                }
...
void dphysics::RigidBodySystem::AdjustVelocity(
    Collision *collision, ysVector velocityChange[2], ysVector rotationChange[2])
{
    ...
    ysVector velKill = ysMath::LoadVector(
        collision->m_desiredDeltaVelocity,
        -ysMath::GetY(collision->m_contactVelocity),
        -ysMath::GetZ(collision->m_contactVelocity));

    ysVector impulseContact = ysMath::MatMult(impulseMatrix, velKill);
    ...
    if (collision->m_bodies[1] != nullptr) {
        impulsiveTorque   = ysMath::Cross(impulse, collision->m_relativePosition[1]);
...
void dphysics::RigidBodySystem::ResolveCollisions(float dt) {
    ...
    while (iterationsUsed < ResolutionIterationLimit) {
        ...
        ResolveCollision(biggestCollision, velocityChange, rotationChange, rotationAmount, max);
        ...
        for (i = 0; i < numContacts; i++) {
            ...
            if (collision.m_body1 == biggestCollision->m_body1) {
                cp = ysMath::Add(ysMath::Cross(rotationChange[0], collision.m_relativePosition[0]), velocityChange[0]);
                collision.m_penetration -= rotationAmount[0] * ysMath::GetScalar(ysMath::Dot(cp, collision.m_normal));
            } else if (collision.m_body1 == biggestCollision->m_body2) {
                cp = ysMath::Add(ysMath::Cross(rotationChange[1], collision.m_relativePosition[0]), velocityChange[1]);
                collision.m_penetration -= rotationAmount[1] * ysMath::GetScalar(ysMath::Dot(cp, collision.m_normal));
            }

            if (collision.m_body2 != nullptr) {
                if (collision.m_body2 == biggestCollision->m_body1) {
                    cp = ysMath::Add(ysMath::Cross(rotationChange[0], collision.m_relativePosition[1]), velocityChange[0]);
                    collision.m_penetration += rotationAmount[0] * ysMath::GetScalar(ysMath::Dot(cp, collision.m_normal));
                } else if (collision.m_body2 == biggestCollision->m_body2) {
                    cp = ysMath::Add(ysMath::Cross(rotationChange[1], collision.m_relativePosition[1]), velocityChange[1]);
                    collision.m_penetration += rotationAmount[1] * ysMath::GetScalar(ysMath::Dot(cp, collision.m_normal));
                }
            }
        }
